...
class DataStore(object):
    ...
    def __init__(store, args):
        ...
        """
        if args.get('datadir') is None:
            args['datadir'] = util.determine_db_dir()
        if isinstance(args.get('datadir'), str):
            args['datadir'] = [args.get('datadir')]

        store.args = args
        store.log = logging.getLogger(__name__)
        store.log.setLevel(logging.DEBUG)

        store.rpclog = logging.getLogger(__name__ + ".rpc")
        if not args.get('log_rpc'):
            store.rpclog.setLevel(logging.ERROR)

        if args.get('dbtype') is None:
            ...
            store._sql = None
            return
        store.dbmodule = __import__(args.get('dbtype'))

        sql_args = lambda: 1
        sql_args.module = store.dbmodule
        sql_args.connect_args = args.get('connect_args')
        sql_args.binary_type = args.get('binary_type')
        sql_args.int_type = args.get('int_type')
        sql_args.log_sql = args.get('log_sql')
        ...
        if store.config is None:
            store.keep_scriptsig = args.get('keep_scriptsig')
        ...
        store.commit_bytes = args.get('commit_bytes')
        ...
        for hex_tx in args.get('import_tx'):
            chain_name = None
        ...
        store.default_loader = args.get('default_loader')
    ...
    def _init_datadirs(store):
        ...
        if store.args.get('datadir') is None:
        ...
        store.datadirs = []
        for dircfg in store.args.get('datadir'):
            loader = None
            conf = None
                ...
                if chain_id is None:
                    ...
                    if chain_id is None and chain_name is not None:
                        ...
                        if script_addr_vers is None:
                            ...
                            script_addr_vers = script_addr_vers.encode(
                                'latin_1')
                            ...
                            INSERT INTO chain (
                                ...
                                chain_decimals
                                  ...
                                  (chain_id, chain_name, code3,
                                   store.binin(addr_vers), store.binin(
                                       script_addr_vers),
    ...
    def init_chains(store):
        ...
        no_bit8_chains = store.args.get('ignore_bit8_chains') or []
...
LEFT JOIN block prev ON (b.prev_block_id = prev.block_id)""",
  ...
  FROM txout""",
    ...
    pid           VARCHAR(255) NULL
        ...
        if store.args.get('use_firstbits') is not None:
        ...
        store.config['keep_scriptsig'] = \
            "true" if store.args.get('keep_scriptsig', False) else "false"
            ...
            INSERT INTO chain (
                ...
                chain_address_version, chain_script_addr_vers, chain_policy, chain_decimals
                  ...
                  (chain.id, store.binin(chain.magic), chain.name,
                   chain.code3, store.binin(chain.address_version), store.binin(
    ...
    def get_lock(store):
        ...
        try:
            import random
            letters = "".join([chr(random.randint(65, 90))
                               for x in xrange(10)])
            store.sql("""
    ...
    def import_block(store, b, chain_ids=None, chain=None):
        ...
        print("Block has {} transactions".format(len(b['transactions'])))
        for pos in xrange(len(b['transactions'])):
            ...
            if tx['tx_id']:
                ...
                if store.commit_bytes == 0:
                    tx['tx_id'] = store.import_and_commit_tx(
                        tx, pos == 0, chain)
                else:
    ...
    def export_block(store, chain=None, block_hash=None, block_number=None):
        ...
        * proof_of_stake_generated
        ...
        if block_number is None and block_hash is None:
            raise ValueError(
                "export_block requires either block_hash or block_number")
        ...
        # Absent the chain argument, default to highest chain_id, preferring to
        # avoid side chains.
        ...
        is_stake_chain = chain is not None and chain.has_feature(
            'nvc_proof_of_stake')
        if is_stake_chain:
            # Proof-of-stake display based loosely on CryptoManiac/novacoin and
            # http://nvc.cryptocoinexplorer.com.
            b['is_proof_of_stake'] = len(tx_ids) > 1 and coinbase_tx[
                'total_out'] == 0
    ...
    def export_address_history(store, address, chain=None, max_rows=-1, types=frozenset(['direct', 'escrow'])):
        ...
        def parse_row(is_out, row_type, nTime, chain_id, height, blk_hash, tx_hash, pos, value, script=None):
            ...
            if script is not None:
                store._export_scriptPubKey(
                    txpoint, chain, store.binout(script))
    ...
    def offer_existing_block(store, hash, chain_id):
        ...
        if store.selectrow("""
            ...
            if b['height'] == 0:
                b['hashPrev'] = store.get_chain_by_id(
                    chain_id).genesis_hash_prev
            else:
            ...
            while rpc_hash is not None:
                ...
                if store.offer_existing_block(hash, chain.id):
                    ...
                    block = {
                        ...
                        'nTime': int(((datetime.strptime(rpc_block['time'], '%Y-%m-%d %H:%M:%S %Z') - datetime(1970, 1, 1)).total_seconds()) if ':' in rpc_block['time'] else rpc_block['time']),
        ...
        while filenum == dircfg['blkfile_number']:
            ...
            if not store.offer_existing_block(hash, chain.id):
                ...
                if b["hashPrev"] == chain.genesis_hash_prev:
                    store.log.info("Genesis block {}".format(str(b)))
                    for k, v in b.items():
                        if isinstance(v, str):
                            store.log.info("{}: {}".format(k, v.encode('hex') or 'None'))
                        else:
                            store.log.info("{}: {}".format(k, v or 'None'))
